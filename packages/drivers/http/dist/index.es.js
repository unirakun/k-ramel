          ...
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                ...
                appliedHeaders = _extends({}, innerOptions.headers, innerHeaders, options.headers);
    ...
    ['GET', 'POST', 'HEAD', 'PUT', 'DELETE', 'OPTIONS', 'CONNECT'].forEach(function (method) {
      ownFetch[method.toLowerCase()] = function (url, data) {
        ...
        if (data && ['object', 'array'].includes(typeof data === 'undefined' ? 'undefined' : _typeof(data))) {
          if (!headers['Content-Type']) appliedOptions = _extends({}, appliedOptions, { headers: _extends({}, headers, { 'Content-Type': 'application/json' }) });

          appliedOptions = _extends({}, appliedOptions, { body: JSON.stringify(data) });
        }

        // set method
        appliedOptions = _extends({}, appliedOptions, { method: method });
